...
class LiqPay
{
    ...
    public function __construct( $api_url = null )
    {
		...
		$this->_public_key 	= get_field( 'liqpay_public_key', 'option' );
		$this->_private_key = get_field( 'liqpay_private_key', 'option' );
	...
	public function liqpay_online_payment_currencies() {
		return array( 1 => 'UAH' );
	}

	public function liqpay_acquiring_statuses() {
		return array( 	2 	=> "failure",
						6 	=> "success",
						25 	=> "processing",
					);
	}

	public function get_maybe_response() {

		if ( isset( $_POST['data'], $_POST['signature'] ) && count( $_POST ) == 2 ) {

			$signature = $this->str_to_sign( $this->_private_key . $_POST['data'] . $this->_private_key );

			if ( $signature == $_POST['signature'] )
				return $this->decode_params( $_POST['data'] );
		}
