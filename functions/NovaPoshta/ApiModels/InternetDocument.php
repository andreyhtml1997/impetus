...
class InternetDocument extends ApiModel
{
    ...
    /**
     * Устанавливает номер ТТН (при получении наложки на карту)
     *
     * @param string $value
     * @return $this
     */
    ...
    private static function getPrintLink($typePrint, MethodParameters $data = null)
    {
	 ...
	 /**
     * Вызвать метод payEwByLoyaltyPoints() - оплатить бонусами ТТН
     *
     * @param MethodParameters $data
     * @return \NovaPoshta\Models\DataContainerResponse
     */
    public static function payEwByLoyaltyPoints(MethodParameters $data = null)
    {
        return self::sendData(__FUNCTION__, $data);
    }
	
    /**
     * Вызвать метод getMoneyTransferDocuments() - получает список наложек
     *
     * @param MethodParameters $data
     * @return \NovaPoshta\Models\DataContainerResponse
     */
    public static function getMoneyTransferDocuments(MethodParameters $data = null)
    {
        return self::sendData(__FUNCTION__, $data);
    }
